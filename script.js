/* TO DO
    ...
    */
    ...
    // function Csurowiec(nazwa,mnoznik)
    // {
    //     this.nazwa = nazwa;
    //     this.mnoznik = mnoznik;
    // }
    // var drewno = Csurowiec('drewno',1);
    // var kamien = Csurowiec('kamien',5);
    ...
    function pasek(budynek)
    {
        ...
        var elem3 = document.getElementById('budynek1')
        budynek.counter++;
    ...
    /*
    function budynekpush()
    {
            if(gracz.budynki >= 1)
            {
                gracz.budynek[0] = budynek1;
            }
            if(gracz.budynki >= 2)
            {
                gracz.budynek[1] = budynek2;
            }
            if(gracz.budynki >= 3)
            {
                gracz.budynek[2] = budynek3;
            }
            if(gracz.budynki >= 4)
            {
                gracz.budynek[3] = budynek4;
            }
            if(gracz.budynki >= 5)
            {
                gracz.budynek[4] = budynek5;
            }
            if(gracz.budynki >= 6)
            {
                gracz.budynek[5] = budynek6;
            }
            if(gracz.budynki >= 7)
            {
                gracz.budynek[6] = budynek7;
            }
            if(gracz.budynki >= 8)
            {
                gracz.budynek[7] = budynek8;
            }
            if(gracz.budynki == 9)
            {
                gracz.budynek[8] = budynek9;
            }      
        } 
        */
    ...
    function stop2()
    {
        for (var i = 0; i+1 <= gracz.budynki; i++) 
        {
            console.log(gracz.budynek[i]);
            clearInterval(gracz.budynek[i].interval);
        }
    }
    function start2() 
    {
        for (var i = 0; i+1 <= gracz.budynki; i++) 
        {
            ...
            gracz.budynek[i].interval = setInterval(() => {pasek(gracz.budynek[i]);}, gracz.budynek[i].pasekczas);
        }
    }
    var pauza4;
    function pauza2()
